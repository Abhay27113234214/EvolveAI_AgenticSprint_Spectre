...
class CFOAssistant {
    ...
    async login(email, password, role) {
        try {
            ...
            if (response.ok) {
                localStorage.setItem('cfo_token', data.token || 'demo_token');
                localStorage.setItem('cfo_user', JSON.stringify({ email, role }));
            ...
            localStorage.setItem('cfo_user', JSON.stringify({ email, role, demo: true }));
            this.isLoggedIn = true;
            this.mockMode = true;
            return { success: true, message: 'Login successful!' };
        }
    ...
    async fetchFinancialData() {
        try {
            ...
            console.log('Using fallback financial data:', error);
    ...
    checkPageAccess() {
        const protectedPages = ['dashboard.html', 'upload.html', 'risks.html', 'insights.html', 'monitoring.html'];
    ...
    setupLoginPage() {
        ...
        if (!form) return;
        // If the form already posts to the Flask backend, do not override with AJAX
        if (form.getAttribute('action')) return;

        // Add role field if not present
        if (!form.querySelector('#role')) {
            this.addRoleFieldToLogin(form);
        }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const role = form.role?.value || 'analyst';
    ...
    setupSignupPage() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const result = await this.signup(name, email, password, role);
    ...
    setupUploadPage() {
        ...
        if (!form.getAttribute('action')) {
            // Form submit via AJAX only when action is not defined
    ...
    renderKPIs(kpis) {
        ...
        const kpiItems = [
            ...
            { key: 'runway', title: 'Runway', icon: '🛤️' },
            { key: 'cash', title: 'Cash Position', icon: '🏦' },
            { key: 'liabilities', title: 'Liabilities', icon: '⚠️' }
        ];
    ...
    addRoleFieldToLogin(form) {
        ...
        const roleHTML = `
            <div class="form-group">
                ...
                <div class="error-message" id="roleError" role="alert"></div>
            </div>
        `;
